...
### Phase E: Advanced Device Models (Step 23–26) — planned

| Step | Concept | What You Learn |
|------|---------|----------------|
| 23 | DMA simulation | Host-to-guest direct memory write |
| 24 | MSI-X emulation | MMIO table + vector selection + masking |
| 25 | PCI config space + BAR | Configuration mechanism, BAR allocation |
| 26 | PCIe hotplug | Slot control state machine + notification |

### Phase F: KVM Internals (Step 27–34) — planned

| Step | Concept | What You Learn |
|------|---------|----------------|
| 27 | VM exit profiler | Exit reason frequency and latency measurement |
| 28 | Exit timeline | Time-series exit pattern visualization |
| 29 | KVM tracepoints | ftrace / perf for KVM internal events |
| 30 | Host+guest correlation | Mapping host-side events to guest behavior |
| 31 | VM exit internals | KVM source: exit handling path |
| 32 | Interrupt virtualization internals | KVM source: LAPIC, posted interrupts |
| 33 | MSR virtualization internals | KVM source: MSR bitmap, emulation |
| 34 | CPUID virtualization internals | KVM source: CPUID filtering |

### Phase G: VT-x / VMCS (Step 35–38) — research topics

| Step | Concept | What You Learn |
|------|---------|----------------|
| 35 | VMCS explorer | VMCS field dump and analysis |
| 36 | EPT explorer | EPT table walk visualization |
| 37 | VM entry/exit controls | VMCS control field experiments |
| 38 | Tiny VMX hypervisor | Minimal VMX implementation without KVM |

> **Note:** Phase G is research-level. Step 38 in particular is a separate project in scope.

## Architecture

```
Phase A–E (Step 1–26):   Build a VMM (How to build a VMM)
Phase F   (Step 27–34):  Observe KVM (How KVM works — from outside)
Phase G   (Step 35–38):  Touch VT-x directly (How VT-x works — from inside)
```

VMCS is last because knowing MMIO, IRQ, MSR, virtio, and migration makes every VMCS field meaningful — "that exit was controlled by *this* field."
