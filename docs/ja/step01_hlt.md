...
>
> Phase A はハードウェア仮想化支援の基礎を扱う: VM の作成、KVM 経由で実 CPU 上で guest コードを実行、guest ↔ host 間の遷移（VM exit）の処理。各ステップが CPU 仮想化の概念を1つずつ追加する。
...
───────────────          ───                         ──────────────
                                                     ...
                                                     9. KVM_SET_REGS
                                                        (RIP=0, RFLAGS=2)
ゲスト実行開始  ←──────── ゲストへ制御を移す ←─────────── 10. ioctl(KVM_RUN)
  │
GPA 0 で hlt 実行
  └── VM exit ────────→ kvm_run に終了理由を記録
                        KVM_RUN から戻る ───────────→ 11. run->exit_reason == 5
                                                     12. 成功メッセージを表示
```
...
sregs に対して GET してから SET する理由: sregs には多くのフィールド (DS, SS, CR0, CR4, EFER…) がある。全てをゼロにすると、ゲストの実行に必要な状態を壊す可能性がある。現在の設定を保持し、必要な部分のみ変更する。
...
`Starting guest...` はゲスト実行開始前の表示。`Exit reason: 5` は `KVM_EXIT_HLT` を示し、最後の行でゲストが `hlt` を実行したことを確認している。

## 重要な知見

VMM が VM の状態を準備し、`KVM_RUN` でゲストを実行して、終了理由を確認するという基本構造を確立する。このステップでは `KVM_RUN` を1回だけ呼び、Step 2 で繰り返し実行するループへ拡張する。
