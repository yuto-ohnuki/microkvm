# Step 28: PCI hotplug — ランタイムでのデバイス追加/削除
...
Linux がバススロットを走査して vendor ID = 0xFFFF を読むと「デバイスなし」を意味する。microkvm では、不在のデバイスへの config read にアクセス幅に応じた all-ones を返すことで、この状態を表現する。
...
microkvm では `present` で config space の可視性を切り替え、手動の `rescan` / `remove` を通じて Linux のデバイス発見、BAR 割り当て、登録解除を観察する。
...
通常のデバイスは `pci_init()` で `present = 1`、2台目は `pci_init_hotplug()` で `present = 0` に初期化する。以下は config read/write の追加部分。
...
config read/write の処理先を、BDF（bus/device/function）に応じて選ぶ。以下は分岐の概略。
...
1台目の BAR0 範囲に該当しない場合、2台目に割り当てられた BAR0 アドレスを調べ、既存の `pci_dev_mmio_read/write()` に振り分ける。
...
> **Note:** `Ctrl-A h` は VMM 側の `present` を切り替える。`rescan` は Linux にバスの再走査を依頼し、`remove` は Linux 側のデバイス登録を解除する。`remove` 自体は `present` を変更しないため、`present = 1` のままなら再度の `rescan` で発見される。

## 重要な知見

このステップの要点は、VMM が返す config space の内容と、Linux が管理するデバイス登録を結び付けること。`present = 1` にして `rescan` すると、Linux は起動時と同様に vendor ID の確認、BAR probing、アドレス割り当てを行う。削除時は `present = 0` で config space を不在にし、`remove` で Linux 側の登録を解除する。
...
```
Step 24: 発見 (config space enumeration)
Step 25: アクセス (BAR 経由のデバイスレジスタ)
Step 26: 転送 (DMA)
Step 27: 通知 (MSI-X 割り込み)
Step 28: ライフサイクル (hotplug 追加/削除)
```
