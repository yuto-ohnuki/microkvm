...
init プロセスが stdin/stdout/stderr を `/dev/ttyS0` (エミュレートされた UART) に接続し、busybox sh を exec する。

## busybox 付き initramfs のビルド

Step 10 の initramfs はメッセージを出力するだけだった。対話型シェルには busybox を追加し init を `/bin/sh` を exec するよう更新:

```bash
$ mkdir -p initramfs/{dev,proc,sys,bin,root}

# スタティック busybox バイナリ (sh, ls, cat 等を提供)
...
# init: ファイルシステムをマウント、ttyS0 接続、シェル exec
