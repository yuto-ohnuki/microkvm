...
このドライバでは、デバイスレジスタへのアクセスを次の順序で準備する:
...
pci_iomap()              → BAR 物理アドレスを kernel 仮想アドレスに map (ioremap)
                           `void __iomem *` を返す — MMIO 用アクセサでアクセス
```

この BAR0 は MMIO 領域で、`pci_iomap(pdev, 0, 0)` は BAR0 全体を map する。最後の引数 0 は長さを BAR 全体とする指定。

### なぜ readl/writel を使うか（ポインタ参照ではダメな理由）

MMIO レジスタは通常のメモリではない:
- read/write に副作用がある場合がある（デバイス状態が変わる）
- コンパイラがアクセスを最適化で削除・並び替えしてはいけない
- アーキテクチャ固有の memory barrier が必要

`readl`/`writel` は 32-bit の MMIO アクセサで、アクセス幅と順序を扱うためのアーキテクチャ固有の処理を提供する。通常のメモリと同じポインタ参照でアクセスせず、これらの API を使う。
...
Device-managed allocation — probe 失敗時、または remove コールバックの終了後に自動解放される。`mdev` に対する `kfree()` は不要だが、`pci_iomap()` と `pci_request_regions()` で得たリソースは明示的に解放する。
...
失敗した段階に応じて `goto` で取得済みのリソースだけを逆順に解放する。正常に probe が完了した場合は、remove で map の解除、BAR 領域の解放、デバイスの無効化を行う。

## 出力

[Step 29 の手順](step29_pci-driver.md#前提条件)に沿って driver を再ビルドし、initramfs 内の `.ko` を差し替えて起動する。
...
`pci_iomap` + `readl` は MMIO アクセスの標準 Linux API。ドライバから見ると単に memory-mapped アドレスを読むだけ。この VM では、EPT による VM exit → KVM の MMIO 処理 → KVM_EXIT_MMIO → VMM のデバイス処理 → 結果返却という経路を通る。ドライバコードは物理ハードウェアでも VM でも同一 — 異なるのは exit パスだけ。
...
| readl/writel | 32-bit MMIO アクセスとアクセス順序の制御 |
| devm_kzalloc | probe 失敗時・remove 終了後に mdev を自動解放 |
...
[Step 31: DMA-capable driver](step31_dma-driver.md) では coherent DMA buffer を確保し、descriptor を構築して doorbell を kick — VMM の DMA エンジンを通じてデータを転送する。
