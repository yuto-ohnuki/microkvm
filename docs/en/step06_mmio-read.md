...
In this step, the VMM maintains a simple `device_counter` variable. Each MMIO write increments the counter, and each MMIO read returns the current value. This demonstrates that the device model can hold **state** that persists across multiple guest accesses - the foundation of any device emulator.
...
The counter increments on each write, so after two writes ('M' and '\n'), a subsequent read returns 2.
