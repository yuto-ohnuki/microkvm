...
```bash
$ git clone --depth 1 --branch v6.19.14 \
  https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git linux-mini
$ cd linux-mini
...
$ cp arch/x86/boot/bzImage /path/to/microkvm/
```

**Note:** Use Linux v6.15+ if building with GCC 15 (Fedora 44+), as older kernels have C23 compatibility issues.

### Building the initramfs

The kernel needs a root filesystem. We create a minimal initramfs with a static init binary:
...
# Static init that prints a message and pauses
$ cat > init.c << 'EOF'
#include <unistd.h>
#include <string.h>
int main(void) {
...
EOF
$ gcc -static -o initramfs/init init.c

# Pack as compressed cpio archive
$ cd initramfs && find . | cpio -o -H newc | gzip > ../initramfs.gz && cd ..
```

The VMM loads `initramfs.gz` into guest memory and passes its address and size via `boot_params`. The kernel unpacks it as the root filesystem and executes `/init` as PID 1.

## Output

```
$ ./microkvm
bzImage: protocol 2.15, setup 16384 bytes, kernel 914432 bytes
Kernel loaded at 0x100000 (914432 bytes)
initramfs loaded at 0x4000000 (2300088 bytes)
Starting guest with 1 vCPUs...
