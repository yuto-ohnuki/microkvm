...
Note: vCPU 1 read `device_counter` first (returning 0), so vCPU 0's later read returned 1. If vCPU 0 had reached the MMIO read first, the values would be reversed. This non-determinism is inherent to concurrent execution.
