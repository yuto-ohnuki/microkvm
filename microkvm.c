...
/* Runtime flags */
static int use_ioeventfd = 0;   /* toggle ioeventfd-based TX kick */
static int use_irqfd = 0;       /* toggle irqfd-based IRQ injection */
...
/* Restore original terminal settings (registered via atexit) */
static void restore_terminal(void) {
    tcsetattr(STDIN_FILENO, TCSANOW, &orig_termios);
}

/* Put stdin into raw mode so guest sees each keystroke immediately */
...
/* Measurement: init, record, report */
...
/* Migration transport: connect and listen */
/*
        ...
        if (virtio_mode) {
            if (virtio_console_rx(&virtio_dev, &c, 1) == 0) {
                ...
                if (use_irqfd) {
                    ...
                    /* Fallback: ioctl-based IRQ injection */
            ...
            if (addr >= VIRTIO_MMIO_BASE && addr < VIRTIO_MMIO_BASE + VIRTIO_MMIO_SIZE) {
                ...
                if (addr == VIRTIO_MMIO_BASE + VIRTIO_MMIO_QUEUE_NOTIFY && run->mmio.is_write) {
                    ...
                    if (nval == 0)
                        ...
                        /* Fallback: TX processing in exit handler */
                        if (!use_ioeventfd) {
                            uint64_t t1 = now_ns();
                            virtio_console_tx(&virtio_dev, virtio_dev.ram, virtio_dev.ram_size);
                            uint64_t t2 = now_ns();
...
int main(int argc, char *argv[]) {
    ...
    /* Enable ioeventfd/irqfd via environment variables */
