...
/* Step 41: the memory regions actually registered with KVM = single source of truth */
...
/* Step 41: find the registered slot backing a GPA (NULL if unregistered gap). */
    ...
    while (read(STDIN_FILENO, &c, 1) == 1) {
        ...
        if (c == 0x01) {
            ...
            if (c == 'e') {
                ...
                /* 1. registered slots (from g_memslots[] = source of truth) */
                for (size_t i = 0; i < g_nr_memslots; i++) {
                ...
                /* 2. GPA gaps between consecutive slots (unregistered ranges).
                 *    We only state the fact "not registered as RAM"; whether a
                 *    gap behaves as MMIO is decided later by KVM (see Case B).
                ...
                /* 3. GPA -> slot -> HVA lookup for two example addresses */
                ...
                for (size_t q = 0; q < sizeof(queries)/sizeof(queries[0]); q++) {
                    ...
                    if (s) {
                        uintptr_t hva = (uintptr_t)s->userspace_addr
                                        + (gpa - s->guest_phys_addr);
                        fprintf(stderr, "  slot : %llu\n  HVA  : 0x%lx\n",
...
int main(int argc, char *argv[]) {
    ...
    /* Register memory with KVM. The registered regions are kept in g_memslots[]
